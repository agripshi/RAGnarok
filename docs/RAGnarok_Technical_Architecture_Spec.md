...
- Add Teams bot capability later if direct chat with the assistant is required.
...
# AI
LLM_PROVIDER=azure_openai_or_openai
LLM_API_KEY=
LLM_ENDPOINT=
LLM_MODEL=
EMBEDDING_MODEL=
...
1. `Come posso richiedere le ferie?`
2. `Qual è la procedura per il congedo per malattia?`
3. `Dove trovo le regole sui benefit?`
